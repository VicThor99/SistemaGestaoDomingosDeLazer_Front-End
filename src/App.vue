...
export default {
  ...
  methods: {
    async submit() {
      await axios.post('http://localhost:8080/api/auth', this.user)
        .then(res => {
          var date = new Date();
          ...
          axios.get('http://localhost:8080/api/access/' + this.username, {
            headers: {
    ...
    download() {
      axios.get('http://localhost:8080/api/alunos/export/' + this.escola, {
        headers: {
