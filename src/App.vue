...
export default {
  ...
  methods: {
    async submit() {
      await axios.post('https://127.0.0.1:8443/api/auth', this.user)
        .then(res => {
          var date = new Date();
          ...
          axios.get('https://127.0.0.1:8443/api/access/' + this.username, {
            headers: {
    ...
    download() {
      axios.get('https://127.0.0.1:8443/api/alunos/export/' + this.escola, {
        headers: {
