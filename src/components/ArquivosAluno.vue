...
export default {
    ...
    methods: {
        async iniciarUpload() {
            ...
            if (this.tipo === 'Foto') {
                axios.post('http://192.168.15.40:8080/api/arquivos/foto/' + this.codigoAluno + '/' + this.escola, formData, {
                    headers: {
                ...
                axios.post('http://192.168.15.40:8080/api/arquivos/matricula/' + this.codigoAluno + '/' + this.escola, formData, {
                    headers: {
                        'Content-Type': 'multipart/form-data',
                        'Authorization': this.token
                    }
