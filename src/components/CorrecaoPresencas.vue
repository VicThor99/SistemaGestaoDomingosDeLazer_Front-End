...
export default {
    ...
    methods: {
        async recarregaLista() {
            const res = await axios.get('http://3.95.225.147:8080/api/registros/' + this.escola, { headers: { 'Authorization': this.token } });
            this.registros = res.data;
        },
        salvarRegistro() {
            axios.post('http://3.95.225.147:8080/api/registros/' + this.registro.codigo + '/'+ this.escola, this.registro, {
                headers: {
