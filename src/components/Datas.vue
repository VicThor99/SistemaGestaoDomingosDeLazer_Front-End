...
export default {
    ...
    methods: {
        async recarregaLista() {
            const res = await axios.get('http://localhost:8080/api/datas/' + this.escola, { headers: { 'Authorization': this.token } });
            this.datas = res.data;
        },
        salvarData() {
            axios.post('http://localhost:8080/api/datas/'+ this.escola, this.dataAula, {
                headers: {
