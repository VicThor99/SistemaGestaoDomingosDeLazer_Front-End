...
export default {
    ...
    methods: {
        async recarregaLista() {
            const res = await axios.get('http://3.95.225.147:8080/api/datas/' + this.escola, { headers: { 'Authorization': this.token } });
            this.datas = res.data;
        },
        salvarData() {
            axios.post('http://3.95.225.147:8080/api/datas/'+ this.escola, this.dataAula, {
                headers: {
