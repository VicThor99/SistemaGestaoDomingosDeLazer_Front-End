...
export default {
    ...
    methods: {
        async recarregaLista() {
            const resAlunos = await axios.get('https://127.0.0.1:8443/api/alunos/' + this.escola, { headers: { 'Authorization': this.token } });
            const resSeries = await axios.get('https://127.0.0.1:8443/api/series/listaString/' + this.escola, { headers: { 'Authorization': this.token } });
        ...
        async salvarAluno() {
            ...
            if (this.aluno.id > 0) {
                await axios.post('https://127.0.0.1:8443/api/registros/' + this.aluno.codigo + '/' + this.escola, this.registro, { headers: { 'Authorization': this.token } }).then(res => {
                    console.log(res);
                    axios.post('https://127.0.0.1:8443/api/alunos/save', this.aluno, {
                        headers: {
                ...
                await axios.post('https://127.0.0.1:8443/api/alunos/save', this.aluno, { headers: { 'Authorization': this.token } }).then(res => {
                    console.log(res);
                    axios.post('https://127.0.0.1:8443/api/registros/' + res.data.codigo + '/' + this.escola, this.registro, {
                        headers: {
                            'Authorization': this.token
                        }
                    }).then(res => {
        ...
        async clickRow(item, row) {
            ...
            await axios.get('https://127.0.0.1:8443/api/registros/' + row.item.columns.codigo + "/" + this.escola, { headers: { 'Authorization': this.token } })
                .then(res => {
                    this.registro = {
            ...
            await axios.get('https://127.0.0.1:8443/api/arquivos/foto/' + row.item.columns.id, {
                headers: {
                    'Authorization': this.token
                }
            })
            ...
            await axios.get('https://127.0.0.1:8443/api/arquivos/matricula/' + row.item.columns.id, {
                headers: {
                    'Authorization': this.token
                }
            })
